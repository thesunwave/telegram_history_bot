...
describe("History Functions Integration Tests", () => {
  ...
  async function createCommandMessages(
    chatId: number,
    count: number,
    startTime: number,
  ): Promise<StoredMessage[]> {
  ...
  describe("fetchMessages function", () => {
    it("should fetch messages for 1 day time range", async () => {
      ...
      const testMessages = await createTestMessages(
        chatId,
        24,
        oneDayAgo,
        3600,
    ...
    it("should handle KV get failures gracefully", async () => {
      ...
      let callCount = 0;
      (vi.spyOn(env.HISTORY, "get") as any).mockImplementation(
        async (key: string, options?: any) => {
          callCount++;
          if (callCount % 3 === 0) {
  ...
  describe("Error scenarios and graceful degradation", () => {
    it("should handle KV list failures in fetchMessages", async () => {
      ...
      vi.spyOn(env.HISTORY, "list").mockRejectedValue(
        new Error("KV list failed"),
      );

      await expect(fetchMessages(env, chatId, oneDayAgo, now)).rejects.toThrow(
        "KV list failed",
      );
    });
    ...
    it("should handle KV list failures in fetchLastMessages", async () => {
      ...
      await expect(fetchLastMessages(env, chatId, 10)).rejects.toThrow(
        "KV list failed",
      );
    });
