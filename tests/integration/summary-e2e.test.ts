...
describe("Summary End-to-End Tests", () => {
   ...
   */
  async function setupLargeMessageSet(
    messageCount: number,
    daysSpread: number = 7,
  ): Promise<void> {
    ...
    for (let i = 0; i < messageCount; i++) {
      ...
      const messageTime =
        startTime + Math.floor((i / messageCount) * daysSpread * dayInSeconds);
   ...
   */
  async function sendSummaryCommand(
    days: number,
  ...
  describe("Large message set processing", () => {
    it("should handle /summary 7 command with 1000 messages without API limit errors", async () => {
      ...
      console.log(
        `Processed 1000 messages in ${responseTime}ms with ${kvRequestCount} KV requests`,
      );
    });
    ...
    it("should handle /summary 7 command with 2000 messages and verify batching limits", async () => {
      ...
      console.log(
        `Processed 2000 messages with max ${maxConcurrentRequests} concurrent requests (batch size: ${env.KV_BATCH_SIZE})`,
      );
    });
  ...
  describe("Concurrent request stability", () => {
    it("should handle 3 concurrent /summary 7 requests without errors", async () => {
      ...
      results.forEach((result, index) => {
        ...
        console.log(
          `Concurrent request ${index + 1} completed in ${result.responseTime}ms`,
        );
      });
    ...
    it("should handle mixed concurrent requests (different periods)", async () => {
      ...
      results.forEach((result, index) => {
        ...
        console.log(
          `Mixed concurrent request ${index + 1} completed in ${result.responseTime}ms`,
        );
      });
