...
describe("Performance Validation and Optimization", () => {
  ...
  describe("Performance comparison before and after batching", () => {
    it("should compare fetchMessages performance with different batch sizes", async () => {
      ...
      results.forEach((result) => {
        ...
        console.log(
          `  Throughput: ${result.metrics.throughput.toFixed(2)} msg/s`,
        );
        console.log(
      ...
      console.log(
        `Optimal batch size for fetchMessages: ${optimalResult.batchSize}`,
      );
      ...
      const minApiRequests = Math.min(
        ...successfulResults.map((r) => r.metrics.apiRequestCount),
      );
    ...
    it("should compare fetchLastMessages performance with different batch sizes", async () => {
      ...
      console.log(
        `Optimal batch size for fetchLastMessages: ${optimalResult.batchSize}`,
      );
    });
  });
  ...
  describe("Performance regression validation for smaller time periods", () => {
    ...
    it("should compare performance scaling from 1-day to 7-day queries", async () => {
      ...
      const oneDayAgo = now - 24 * 3600;
      apiRequestCounter = 0;
      const { result: result1Day, metrics: metrics1Day } =
      ...
      // Test 7-day query
      apiRequestCounter = 0;
      const { result: result7Days, metrics: metrics7Days } =
        await measurePerformance(
          () => fetchMessages(env, chatId, sevenDaysAgo, now),
          168,
        );
      ...
      const apiRequestRatio =
        metrics7Days.apiRequestCount / metrics1Day.apiRequestCount;
      ...
      console.log(
        `1-Day Query: ${metrics1Day.duration.toFixed(2)}ms, ${metrics1Day.apiRequestCount} requests`,
      );
      console.log(
        `7-Day Query: ${metrics7Days.duration.toFixed(2)}ms, ${metrics7Days.apiRequestCount} requests`,
      );
      console.log(
        `Duration Scaling: ${durationRatio.toFixed(2)}x (expected ~${scalingFactor.toFixed(2)}x)`,
      );
      console.log(
        `API Request Scaling: ${apiRequestRatio.toFixed(2)}x (expected ~${scalingFactor.toFixed(2)}x)`,
      );
    });
  });
  ...
  describe("Optimal batch size determination", () => {
    it("should find optimal batch size for various message counts", async () => {
      ...
      for (const messageCount of messageCounts) {
        ...
        if (successfulResults.length > 0) {
          const optimal = successfulResults.reduce((best, current) =>
            current.metrics.throughput > best.metrics.throughput
              ? current
              : best,
          );
      ...
      optimalBatchSizes.forEach(
        ({ messageCount, optimalBatchSize, metrics }) => {
          console.log(
            `${messageCount} messages: optimal batch size ${optimalBatchSize}`,
          );
          console.log(`  Throughput: ${metrics.throughput.toFixed(2)} msg/s`);
          console.log(`  Duration: ${metrics.duration.toFixed(2)}ms`);
          console.log(`  API Requests: ${metrics.apiRequestCount}`);
    ...
    it("should validate batch size impact on API request patterns", async () => {
      ...
      apiRequestPatterns.forEach(({ batchSize, totalRequests, efficiency }) => {
        console.log(
          `Batch Size ${batchSize}: ${totalRequests} requests, ${efficiency.toFixed(2)} msg/req`,
        );
      });

      // Validate that larger batch sizes generally result in fewer total requests
      const sortedByBatchSize = [...apiRequestPatterns].sort(
      ...
      const maxEfficiency = Math.max(
        ...apiRequestPatterns.map((p) => p.efficiency),
      );
      const minRequests = Math.min(
        ...apiRequestPatterns.map((p) => p.totalRequests),
      );
  ...
  describe("API request count validation", () => {
    it("should measure actual API request counts in test scenarios", async () => {
      ...
      const thirtyDayResult = results.find((r) =>
        r.scenario.includes("30-day"),
      );

      if (sevenDayResult && thirtyDayResult) {
        const requestRatio =
          thirtyDayResult.apiRequests / sevenDayResult.apiRequests;
    ...
    it("should validate API request limits are not exceeded", async () => {
      ...
      for (const batchSize of batchSizes) {
        ...
        console.log(
          `Batch size ${batchSize}: ${apiRequestCounter} API requests for ${messages.length} messages`,
        );
      }
    });
  });
  ...
  describe("Performance under different conditions", () => {
    ...
    it("should validate performance consistency across multiple runs", async () => {
      ...
      console.log(
        `Average Duration: ${avgDuration.toFixed(2)}ms (±${durationStdDev.toFixed(2)})`,
      );
  ...
  describe("Batch processing utility performance", () => {
    it("should validate processBatches performance characteristics", async () => {
      ...
      for (const itemCount of itemCounts) {
        ...
        for (const batchSize of batchSizes) {
          ...
          console.log(
            `  Batch Size ${batchSize}: ${duration.toFixed(2)}ms, ${throughput.toFixed(2)} items/s`,
          );
        }
      }
    });
    ...
    it("should validate processBatchesDetailed performance and metrics", async () => {
      ...
      expect(result.metrics.totalBatches).toBe(
        Math.ceil(itemCount / batchSize),
      );
      ...
      console.log(
        `Batch Metrics Duration: ${result.metrics.totalDuration.toFixed(2)}ms`,
      );
      ...
      console.log(
        `Average Batch Duration: ${result.metrics.averageBatchDuration.toFixed(2)}ms`,
      );
      console.log(`Total Batches: ${result.metrics.totalBatches}`);
      console.log(
        `Successful Results: ${result.results.filter((r) => r !== null).length}`,
      );
