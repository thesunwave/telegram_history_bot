  ...
  formatUserStats(stats: UserStats): string {
    ...
    if (stats.violationsByArticle && stats.violationsByArticle.length > 0) {
      ...
      if (sortedViolations.length > 0) {
        sortedViolations.forEach(({ article, subarticle, articleTitle, punishment, count, averageSeverity }) => {
          try {
            ...
            const fullArticle = formatFullArticle(article, subarticle);
            lines.push(`• ${this.htmlBuilder.bold(`${formatArticleForDisplay(fullArticle)}`)} ${count || 0} раз (ср. ${(averageSeverity || 0).toFixed(1)})`);
          }
        });
      } else {
