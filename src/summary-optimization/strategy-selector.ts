...
import { ProcessingStrategySelector, ProcessingStrategy, SummaryOptimizationConfig } from './types';
...
export class OptimizedStrategySelector implements ProcessingStrategySelector {
  ...
  shouldUseParallelProcessing(estimatedMessageCount: number): boolean {
    ...
    const shouldUse = estimatedMessageCount >= this.config.parallelProcessing.minMessagesThreshold;
  ...
  shouldUseHierarchicalProcessing(tokenCount: number): boolean {
    ...
    const shouldUse = tokenCount > this.config.hierarchicalProcessing.chunkSizeThreshold;
    
    Logger.debug(this.env, 'Strategy selector: hierarchical processing decision', {
      tokenCount,
  ...
  getOptimalWorkerCount(messageCount: number): number {
    ...
    Logger.debug(this.env, 'Strategy selector: optimal worker count calculation', {
      messageCount,
      batchSize,
      maxWorkers,
      idealWorkers,
  ...
  selectStrategy(messageCount: number, estimatedTokens: number): ProcessingStrategy {
    Logger.debug(this.env, 'Strategy selector: selecting processing strategy', {
      ...
      config: {
        ...
        hierarchicalThreshold: this.config.hierarchicalProcessing.chunkSizeThreshold
      }
    });

    // Decision tree for strategy selection
    const useParallel = this.shouldUseParallelProcessing(messageCount);
    const useHierarchical = this.shouldUseHierarchicalProcessing(estimatedTokens);
   ...
   */
  explainStrategySelection(messageCount: number, estimatedTokens: number): {
    ...
    if (estimatedTokens > this.config.hierarchicalProcessing.chunkSizeThreshold) {
      reasoning.push(`Token count (${estimatedTokens}) exceeds hierarchical threshold (${this.config.hierarchicalProcessing.chunkSizeThreshold})`);
    }
    
    if (messageCount >= this.config.parallelProcessing.minMessagesThreshold) {
      reasoning.push(`Message count (${messageCount}) meets parallel processing threshold (${this.config.parallelProcessing.minMessagesThreshold})`);
    } else {
      reasoning.push(`Message count (${messageCount}) below parallel processing threshold (${this.config.parallelProcessing.minMessagesThreshold})`);
    }
